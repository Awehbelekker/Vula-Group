...
TOOL_SPECS: List[Dict[str, Any]] = [
    ...
    {"type": "function", "function": {
        ...
        "description": "Search filed documents (invoices, quotes, proof-of-payment, BOQs, "
                       ...
                       "invoice number, an amount, what it was for. Use this when the owner "
                       ...
                       "acting or answering — AND ALSO for a broader request naming a supplier/"
                       "customer, e.g. 'all invoices from X', 'what have we spent/paid with X', "
                       "'summary of X spend': this is a real transaction-history lookup over "
...
KNOWLEDGE_TOOLS = [
    {"type": "function", "function": {
        ...
        "description": (
            ...
            "different, rival business (a competitor) — use competitor_check for that. Do NOT "
            "use this for what the business has actually spent, paid, or been invoiced by a "
            "named supplier ('what have we spent with X', 'invoices from X') — that's real "
            "transaction history, not pricing/catalog info; use find_document (then "
            "email_thread_summary) for that instead."
        ),
        "parameters": {"type": "object", "properties": {
...
class CommerceAdminSkill(BaseSkill):
    ...
    def _system_prompt(self, tenant_id: str, role: Optional[str] = None, name: Optional[str] = None,
        ...
        return (
            ...
            "BEFORE acting or answering. The same applies to a broader request naming a "
            "supplier/customer — 'all invoices from X', 'what have we spent/paid with X' — that's "
            "real transaction history over actual filed documents, so it goes to find_document "
            "(then email_thread_summary on a miss) too, never lookup_business_info, which only "
            "knows what THIS business charges/sells, not what it has actually spent or been "
            "invoiced. This does NOT apply to a request to CREATE something "
