...
TOOL_SPECS: List[Dict[str, Any]] = [
    ...
    {"type": "function", "function": {
        ...
        "description": "Search documents ALREADY FILED from past emails (invoices, quotes, "
                       ...
                       "Also for 'how much have we spent with X' / 'what materials did we buy "
                       "from X': found invoices come back with total_matches, a server-computed "
                       "total_amount (refunds already subtracted) and a `materials` roll-up of "
                       "their line items (quantities and spend already summed) — quote those "
                       "as-is instead of adding up the list yourself, and follow the result's "
                       "note (e.g. amounts missing from the total, a quantity to confirm, a "
                       "supplier name it inferred that the user should confirm). If this returns "
...
async def _direct_supplier_answer(question: str, tool: str, args: Dict[str, Any], result: Any,
    ...
    2026-09-26: `phone` (the WhatsApp number to send a real .xlsx to, when the question
    explicitly asks for excel/spreadsheet/csv — see service.send_supplier_history_xlsx) flows
    through to whichever path answers."""
    ...
    return await answer_supplier_history_continuation(tenant_id, history, question or "", phone=phone)
    ...
    async def run(self, inp: SkillInput) -> SkillOutput:
        ...
        if looks_like_supplier_history_question(inp.question or ""):
            try:
                ...
                direct = await answer_supplier_history(inp.tenant_id, inp.question, phone=phone)
