"""
core/skills/microsoft_admin.py — OneDrive + Outlook over WhatsApp/portal.

    "find the Bokaap fee proposal in onedrive"  → drive_search
    "pull that file in"                          → drive_pull (downloads + files into KB)
    "any new emails from the client?"            → mail_list / mail_read
    "draft a reply confirming the meeting"        → mail_draft (DRAFT only — never sends)
...
TOOL_SPECS: List[Dict[str, Any]] = [
    {"type": "function", "function": {
        "name": "drive_search", "description": "Search the user's OneDrive by file name.",
        "parameters": {"type": "object", "properties": {
            "query": {"type": "string"}}, "required": ["query"]}}},
    {"type": "function", "function": {
        "name": "drive_pull",
        "description": "Download a OneDrive file (by id from drive_search) and file it into the "
                       "knowledge base so the assistant can answer questions about it.",
        "parameters": {"type": "object", "properties": {
            "file_id": {"type": "string"}, "name": {"type": "string"}}, "required": ["file_id"]}}},
    {"type": "function", "function": {
        "name": "mail_list", "description": "List recent Outlook emails (optional search query).",
        "parameters": {"type": "object", "properties": {"query": {"type": "string"}}}}},
    {"type": "function", "function": {
        "name": "mail_read", "description": "Read a full email by id (from mail_list).",
        "parameters": {"type": "object", "properties": {
            "message_id": {"type": "string"}}, "required": ["message_id"]}}},
    {"type": "function", "function": {
        "name": "mail_draft",
        "description": "Create an Outlook DRAFT (it is NOT sent — the user reviews and sends).",
        "parameters": {"type": "object", "properties": {
...
class MicrosoftAdminSkill(BaseSkill):
    name = "microsoft_admin"
    description = "Search/file OneDrive documents and read/draft Outlook mail (draft-only)."

    async def run(self, inp: SkillInput) -> SkillOutput:
        from vula.microsoft.credentials import get_access_token
        if not await get_access_token(inp.tenant_id):
            return SkillOutput(
                answer="Microsoft isn't connected yet. Connect OneDrive & Outlook in Settings, "
        ...
        try:
            ...
            logger.warning("microsoft_admin failed: %s", exc)
    ...
    async def _dispatch(self, name: str, args: Dict[str, Any], tenant_id: str) -> Any:
        try:
            ...
            if name == "mail_list":
                return {"emails": await service.mail_list(tenant_id, args.get("query", ""))}
            if name == "mail_read":
                return await service.mail_read(tenant_id, args.get("message_id", ""))
            if name == "mail_draft":
                return await service.mail_create_draft(tenant_id, args.get("to", ""),
                    args.get("subject", ""), args.get("body", ""))
        except MicrosoftNotConnected:
            return {"error": "Microsoft not connected."}
        except Exception as exc:
            logger.warning("microsoft tool %s failed: %s", name, exc)
