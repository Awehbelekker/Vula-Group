...
from core.skills.base import (
    BaseSkill, SkillInput, SkillOutput, behaviour_preamble, need_info_message, wrong_arithmetic,
)
...
async def draft_letter(args: Dict[str, Any], tenant_id: str, phone: str,
    ...
    # 2026-09-26: this content is generated by a separate, unchecked LLM call
    # (vula.api.draft._generate_document) and rendered straight to a real signed PDF/docx the
    # owner sends to a client — higher stakes than a chat reply, since draft_admin's own _loop
    # never restates these figures for _dispatch's need_info/direct-answer checks to catch. Same
    # deterministic backstop commerce_admin.py/commerce_assistant.py use for a chat answer,
    # applied here to the document text itself; same has_placeholders shape (a flag on the tool
    # result, surfaced via the system prompt below) since there's no chat turn left to rewrite
    # an already-generated document.
    bad_maths = wrong_arithmetic(content)
    has_bad_arithmetic = bool(bad_maths)
    if has_bad_arithmetic:
        logger.warning("draft_admin WRONG ARITHMETIC in generated document, tenant=%s: %s",
                       tenant_id, bad_maths)
    ...
    result: Dict[str, Any] = {"draft_id": draft_id, "document_type": doc_type,
                              ...
                              "has_bad_arithmetic": has_bad_arithmetic, "output_format": ext}
...
class DraftAdminSkill(BaseSkill):
    ...
    def _system(self) -> str:
        return ("You are Vula, drafting professional business letters for a South African "
                ...
                "- If draft_letter's result has has_bad_arithmetic:true, tell the user the draft "
                "has a calculation that doesn't add up and to double-check the figures before "
                "sending it on — same as placeholders, don't present it as complete.\n"
