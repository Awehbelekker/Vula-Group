...
# ── deterministic arithmetic backstop on the generated document (2026-09-26) ───────────
# draft_letter's content is generated by a separate, unchecked LLM call and rendered straight
# to a real signed PDF the owner sends to a client — same failure class wrong_arithmetic()
# already catches in a chat reply (real incident: "11.8 x 18.2 = 215.56", correct 214.76),
# applied here since there's no chat turn left to rewrite an already-generated document.

@pytest.mark.asyncio
async def test_draft_letter_flags_bad_arithmetic_in_output():
    brief = ("Fee proposal, R2,500,000, 450 m2, concept and documentation, 8% of "
             "construction cost.")
    with (
        patch("vula.api.draft._retrieve_context", new=AsyncMock(return_value=("", 0))),
        patch("vula.api.draft._generate_document",
              new=AsyncMock(return_value=("Area: 11.8 x 18.2 = 215.56 sqm.", "test-model"))),
        patch("vula.commerce.service.get_invoice_settings", new=AsyncMock(return_value={})),
        patch("vula.commerce.pdf.merge_branding", return_value={"name": "DIGG Architects"}),
        patch("vula.commerce.pdf.render_letter_pdf", return_value=b"%PDF-fake"),
    ):
    ...
    assert result["has_bad_arithmetic"] is True


@pytest.mark.asyncio
async def test_draft_letter_no_bad_arithmetic_flag_when_correct():
    brief = ("Fee proposal, R2,500,000, 450 m2, concept and documentation, 8% of "
             "construction cost.")
    with (
        patch("vula.api.draft._retrieve_context", new=AsyncMock(return_value=("", 0))),
        patch("vula.api.draft._generate_document",
              new=AsyncMock(return_value=("Area: 11.8 x 18.2 = 214.76 sqm.", "test-model"))),
        patch("vula.commerce.service.get_invoice_settings", new=AsyncMock(return_value={})),
        patch("vula.commerce.pdf.merge_branding", return_value={"name": "DIGG Architects"}),
        patch("vula.commerce.pdf.render_letter_pdf", return_value=b"%PDF-fake"),
    ):
    ...
    assert result["has_bad_arithmetic"] is False


def test_system_prompt_tells_the_model_to_flag_bad_arithmetic():
    from core.skills.draft_admin import DraftAdminSkill
    prompt = DraftAdminSkill()._system()
    assert "has_bad_arithmetic" in prompt
    assert "double-check the figures" in prompt
