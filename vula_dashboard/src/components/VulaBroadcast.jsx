  ...
  async function sendBroadcast() {
    ...
    try {
      ...
      const r = await fetch(`${VULA_API}/v1/commerce/${tenantId}/admin/broadcasts/send`, {
        ...
        body: JSON.stringify({ template_name: template, audience_filter: audience, name: tpl?.label }),
      })
      const d = await r.json()
  ...
  return (
    <div>
      ...
      <div style={s.composeCard}>
        ...
        {sent && <p style={s.success}>Broadcast sent successfully!</p>}

        <div style={s.preview}>
          <p style={s.previewLabel}>Preview</p>
          <p style={s.previewText}>
            Sending <strong>{selectedTpl?.label}</strong> to <strong>{selectedAud?.label}</strong>
          </p>
        </div>

        <button onClick={sendBroadcast} disabled={sending} style={sending ? s.btnDisabled : s.sendBtn}>
...
const s = {
  ...
  sendBtn:      { width: '100%', padding: '12px', background: '#25D366', color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'system-ui' },
