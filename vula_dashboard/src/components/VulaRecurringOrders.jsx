...
function NewSub({ tenantId, products, onDone, flash }) {
  const [f, setF] = useState({ customer_name: "", customer_phone: "", cadence: "weekly", payment_method: "cod" });
  ...
  const save = async () => {
    ...
    if (!f.customer_phone.trim()) return flash("Customer phone is required");
    setSaving(true);
    const r = await fetch(`${VULA_API}/v1/subscriptions/${tenantId}`, {
  ...
  return (
    <div style={{ ...card, flexDirection: "column", alignItems: "stretch", gap: 10, background: C.alt }}>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        ...
        <input placeholder="Phone (27…)" value={f.customer_phone} onChange={e => setF({ ...f, customer_phone: e.target.value })} style={input} />
