...
async def run_task(goal: str, enable_mesh: bool = False) -> str:
    ...
    if enable_mesh:
        ...
        hrm.update_mesh_state(discovery.mesh_state)

    print(f"\n🧠 Universal Soul — processing: {goal[:80]}")
    print("─" * 60)

    # Step 1: HRM builds the task graph
    graph = hrm.build_graph(goal)
    print(
        f"📊 Complexity: {graph.complexity_label} ({graph.complexity:.0%}) | "
    ...
    merged = merger.merge(graph)
    graph.mark_done(merged)
